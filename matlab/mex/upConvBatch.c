...
#define DEBUG_SHOW_TYPE(type, name, var) printf(">>> %s = " type "\n", name, (var));
#define DEBUG_SHOW_D(var) DEBUG_SHOW_TYPE("%d", #var, var);
...
void mexFunction(int nlhs,	     /* Num return vals on lhs */
  ...
  mwSize * idims, * rdims6, rdims[3];
  int x_fdim, y_fdim, x_idim, y_idim, z_idim;
  int orig_x = 0, orig_y, x, y, z;
  int x_rdim, y_rdim, z_rdim;
  int x_start = 1;
  int x_step = 1;
  int y_start = 1;
  int y_step = 1;
  ...
  idims = mxGetDimensions(arg0);
  x_idim = (int) idims[0]; /* X is inner index! */
  y_idim = (int) idims[1];
  z_idim = 1;
  if (mxGetNumberOfDimensions(arg0) > 2) {
      z_idim = (int) idims[2];
  }
  ...
  if (nrhs>6)
	 ...
	 manager, so we don't return anything if the result image is passed */
      ...
      rdims6 = mxGetDimensions(arg6);
      
      x_rdim =  (int) rdims6[0]; /* X is inner index! */
      y_rdim = (int) rdims6[1];
      z_rdim = 1;
      if (mxGetNumberOfDimensions(arg6) > 2) {
        z_rdim = (int) rdims6[2];
      }
	...
	mexErrMsgTxt("STOP values must within image dimensions.");
      ...
      /* OLD plhs[0] = (mxArray *) mxCreateDoubleMatrix(x_rdim,y_rdim,mxREAL); */
      rdims[0] = x_rdim;
      rdims[1] = y_rdim;
      rdims[2] = z_rdim;
      plhs[0] = (mxArray *) mxCreateNumericArray( 3, rdims, mxDOUBLE_CLASS, mxREAL );
  ...
  if (strcmp(edges,"circular") == 0)
  {
    ...
    for(z = 0; z < z_rdim; ++z)
    { 
        double *thread_image = image + (x_idim*y_idim) * z;
        double *thread_result = result + (x_rdim*y_rdim) * z;
        
        internal_wrap_expand(thread_image, filt, x_fdim, y_fdim,
                     x_start, x_step, x_stop, y_start, y_step, y_stop,
                     thread_result, x_rdim, y_rdim);
    }
  }
  else
  {
    temp = mxCalloc(x_fdim * y_fdim * omp_get_max_threads(), sizeof(double));
    if (temp == NULL) {
        mexErrMsgTxt("Cannot allocate necessary temporary space");
        return;
    }
    ...
    { 
        double *thread_image = image + (x_idim*y_idim) * z;
        double *thread_result = result + (x_rdim*y_rdim) * z;
        double *thread_temp = temp + (x_fdim*y_fdim) * omp_get_thread_num();
        
      internal_expand(thread_image, filt, thread_temp, x_fdim, y_fdim,
		       x_start, x_step, x_stop, y_start, y_step, y_stop,
		       thread_result, x_rdim, y_rdim, edges);
    }
