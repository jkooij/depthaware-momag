...
void mexFunction(int nlhs,	     /* Num return vals on lhs */
  ...
  mwSize * idims, rdims[3];
  int x_fdim, y_fdim, x_idim, y_idim, z_idim;
  int x_rdim, y_rdim, z_rdim, z;
  int x_start = 1;
  int x_step = 1;
  int y_start = 1;
  int y_step = 1;
  ...
  if (strcmp(edges,"circular") == 0)
  {
    ...
    for(z = 0; z < z_rdim; ++z)
    {
        /* mexPrintf("Num threads %d, thread ID %d, z = %d.\n", omp_get_num_threads(), omp_get_thread_num(), z); */
        
        double *thread_image = image + (x_idim*y_idim) * z;
        double *thread_result = result + (x_rdim*y_rdim) * z;

        internal_wrap_reduce(thread_image, x_idim, y_idim, filt, x_fdim, y_fdim,
  ...
  else 
  {
    ...
    /* printf("OMP is using %d threads of %d\n", omp_get_num_threads(), omp_get_max_threads()); */
    ...
    for(z = 0; z < z_rdim; ++z)
    {
        ...
        internal_reduce(thread_image, x_idim, y_idim, filt, thread_temp, x_fdim, y_fdim,
               x_start, x_step, x_stop, y_start, y_step, y_stop,
               thread_result, edges);
    }
